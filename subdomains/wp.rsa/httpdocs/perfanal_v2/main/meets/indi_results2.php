...
$meta_tags.='<meta name="robots" content="index, nofollow">';
		  ...
		  $query = "(select e.MtEvent, e.MtEv, e.MtEvX, e.Lo_Hi, e.Sex as ESex, e.Distance, e.Stroke,e.I_R, e.Course, round(r.OfBest/100,2) AS improv, a.Athlete, a.Last, a.First, a.Sex,r.F_P , If(r.PLACE>0,r.PLACE,'') As PLACE,r.MTEVENT, r.NT, r.SCORE,IF(r.POINTS>0,Round(r.POINTS/10),'') as POINTS, r.Age, t.TCode, t.LSC";
		  $query.= " from (((".$db_name."mtevent as e left join ".$db_name."result as r on (e.MtEvent=r.MTEVENT))) inner join ".$db_name."athlete as a on (r.ATHLETE= a.Athlete)) left join ".$db_name."team as t on (r.TEAM=t.Team) ";
		  $query.= $Where." order by e.Meet,e.MtEv,e.MtEvX, r.F_P,r.NT,r.PLACE,r.SCORE)";
