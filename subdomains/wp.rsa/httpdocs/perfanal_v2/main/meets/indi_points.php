...
$query = "select m.MName, e.MtEv, e.MtEvX, e.Lo_Hi, e.Sex as ESex, e.Distance, e.Stroke, e.Course from ".$db_name."mtevent e inner join ".$db_name."meet m on (e.Meet=m.Meet) WHERE e.Meet=".inj_int($_GET['m'])." ";
   ...
   $query.=" ( ".$db_name."result as r inner join ".$db_name."athlete as a on (r.ATHLETE= a.Athlete)) left join ".$db_name."team as t on (r.TEAM=t.Team)";
   ...
   while ($object = mysql_fetch_object($result))
	...
	$rows[] = array((($pos ==$pos2)?'-':$pos),Round($object->Sum_Points,1),l2($object->Last.",&nbsp;".$object->First, $link,'../athlete/times/top_times.php'), $object->Sex, $object->age, $object->TCode."-".$object->LSC);
	if($pos != $pos2)
	  $pos2 = $pos;
     }
   if (!$rows)
