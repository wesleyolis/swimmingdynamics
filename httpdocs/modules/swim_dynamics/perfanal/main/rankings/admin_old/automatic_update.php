		 ...
		 $ranking_stage++;
	       ...
	       if($ranking_stage <3)
	       {
	        if($rank_fina=='y')
			  ...
			  $query.=" set r.RFINA = if((@".$grp." != (@".$grp.":=r.athlete+r.COURSE+r.STROKE+r.distance+r.NT+r.I_R) and r.NT=0 and r.I_R = 'I'),1,if(r.points!=0,2,0))";
		...
		$ranking_stage++;
	     ...
	     if($meet_type !='ALL' & $meet_type !='LSC' & $meet_type_fina !='ALL' & $meet_type_fina !='LSC')//$meet_type !='' 
		  ...
		  if($ranking_stage <6)
			...
			if($i==1)
			{
			$query.=" set r.Rtype=if(r.RAll<0,-1,c.tindex)";
			}
			else
			{
			$query.=" set r.Rtype=if(r.Rtype=0,c.tindex,r.Rtype)";
			}
			
			for($i2=2;$i2<=$i;$i2++)
			{
			$query.=",r.Rtype".$i2."=if(r.Rtype".(($i2==2)?'':$i2)."=0,0,c.tindex)";
			}
		 ...
		 while($object = db_fetch_object($results))
		  {
			...
			//  db_query('INSERT INTO TargetTable (result,sub) ');
		        db_query("set @".$grp."='0'");
		       $query=" update ".$tm4db."result_".$season." as r";
		       $query.=" set r.RType = if(r.RType!=".$object->tindex.",r.RType,if((@".$grp." != (@".$grp.":=r.athlete+r.COURSE+r.STROKE+r.distance+r.NT+r.I_R) and r.NT=0 and r.I_R = 'I'),".$object->tindex.",if(r.POINTS!=0,".($object->tindex+1).",0)))";
		      
		       $col_Rtype='';
		       for($i=2;$i<=$max_sanctions;$i++)
		       {
			       $query.=", r.RType".$i." = if(r.RType".$i."!=".$object->tindex.",r.RType".$i.",if((@".$grp." != (@".$grp.":=r.athlete+r.COURSE+r.STROKE+r.distance+r.NT+r.I_R) and r.NT=0 and r.I_R = 'I'),".$object->tindex.",if(r.POINTS!=0,".($object->tindex+1).",0)))";
