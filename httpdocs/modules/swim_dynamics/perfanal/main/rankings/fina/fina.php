...
$url = 'ranking/'.arg(1).'/'.arg(2).'/'.arg(3).'/'.arg(4).'/'.arg(5).'/'.arg(6).'/'.arg(7).'/'.arg(8).'/'.arg(9).'/'.arg(10);
 if($rank_fina=='y')
 {
	 ...
	 if(arg(11)=='next')
	 {
		 ...
		 drupal_set_title($pref_head.' '.$type[arg(3)].' '.$fina_year.' Rankings'." for ".(arg(1)).'-'.$rank_month.'-'.$rank_day.' to '.(arg(1)+2).'-'.$rank_month.'-'.$rank_day.' ('.$last_ranking.")<br/><small>".$heading.' '.Gender(arg(7)).' '.Age(arg(8)).' '.((arg(9)=='All' & arg(10)=='All')?'Overall':(((arg(10)=='All')?'':arg(10).'m ').((arg(9)=='All')?'':Stroke(arg(9))))).' '.Course(0,arg(6)).', Ages on \''.(arg(1)+1).'-'.$rank_month.'-'.$rank_day.'\'.</small>'.((arg(2)=='curr')?'':"<small><br>Athletes ages on ".arg(2).'</small>'));
	 ...
	 else
	 {
		 ...
		 drupal_set_title($pref_head.' '.$type[arg(3)].' '.$fina_year.' Rankings'." for ".(arg(1)-1).'-'.$rank_month.'-'.$rank_day.' to '.(arg(1)+1).'-'.$rank_month.'-'.$rank_day.' ('.$last_ranking.")<br/><small>".$heading.' '.Gender(arg(7)).' '.Age(arg(8)).' '.((arg(9)=='All' & arg(10)=='All')?'Overall':(((arg(10)=='All')?'':arg(10).'m ').((arg(9)=='All')?'':Stroke(arg(9))))).' '.Course(0,arg(6)).', Ages on \''.arg(1).'-'.$rank_month.'-'.$rank_day.'\'.</small>'.((arg(2)=='curr')?'':"<small><br>Athletes ages on ".arg(2).'</small>'));
 ...
 {
  drupal_set_title($pref_head.' '.$type[arg(3)].' '.$fina_year.' Rankings'." as of ".$last_ranking."<br/><small>".$heading.' '.Gender(arg(7)).' '.Age(arg(8)).' '.((arg(9)=='All' & arg(10)=='All')?'Overall':(((arg(10)=='All')?'':arg(10).'m ').((arg(9)=='All')?'':Stroke(arg(9))))).' '.Course(0,arg(6)).'</small>'.((arg(2)=='curr')?'':"<small><br>Athletes ages on ".arg(2).'</small>'));//, Ages as of day on meet.
 }
		  if(arg(9)!='All')
 ...
 $sort = (arg(12)==null)?0:arg(12);
   ...
   $query.= "a.Athlete, a.Last, a.First, a.Sex, ".($rank_pre_seasons?'r.age as age':$ath_age).", t.TCode, t.LSC FROM (((".$tm4db."result_".$season." as r left join ".$tm4db."athlete_".$season." as a on (r.ATHLETE=a.Athlete)) ".$filter_team." ) left join ".$tm4db."meet_".$season." as m on (r.Meet=m.Meet)) WHERE r.fina!=0 and ".$Where.' '.$Where1.' '.$Where_rank[0];
