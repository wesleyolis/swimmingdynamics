				  ...
				  if($object->Course=='LO' ||$object->Course=='LS' ||$object->Course=='SL' ||$object->Course=='SO' || $object->Course=='S' || $object->Course=='L')
				    {
				        ...
				        if($object->Course=='LO')
					 {
					    ...
					    $query.=" SELECT r.I_R,e.F".$std_qt.",e.S".$std_qt.",e.F".($std_qt+12).",e.S".($std_qt+12).",((e.Lo_Age*100)+e.Hi_Age) as Lo_Hi,e.Sex,e.Stroke,e.Distance,r.SCORE,r.Course,m.MName,m.Start,r.Score as conv,1 as pref,If((e.S".($std_qt+12).">0),If(e.S".($std_qt+12).">=r.Score,11,12),13) + If((e.F".($std_qt+12).">0),If(e.F".($std_qt+12)."<=r.Score,0,-10),0) as qt ";
				       ...
				       if($object->Course=='LS') //Considers Muti course cut meets
					 {
					    ...
					    //long course query
					    $query.=" SELECT r.I_R,e.F".$std_qt.",e.S".$std_qt.",e.F".($std_qt+12).",e.S".($std_qt+12).",((e.Lo_Age*100)+e.Hi_Age) as Lo_Hi,e.Sex,e.Stroke,e.Distance,r.SCORE,r.Course,m.MName,m.Start,r.Score as conv,1 as pref,If((e.S".($std_qt+12).">0) Or (e.S".$std_qt.">0),If(e.S".($std_qt+12).">0,If(e.S".($std_qt+12).">=r.Score,11,12),13),13) + If((e.F".$std_qt.">0) Or (e.F".($std_qt+12).">0),If((e.F".($std_qt+12).">0),If(e.F".($std_qt+12)."<=r.Score,0,-10),10),0) as qt ";
				       ...
				       if($object->Course=='L') //Covert all times to long course then Consider them, note preferance (pref is equal here =1)
					 {
						 $qtset="  (e.F".($std_qt+12).">0) Or (e.S".($std_qt+12).">0) ";
					    //long course query
					    $query.=" SELECT r.I_R,e.F".$std_qt.",e.S".$std_qt.",e.F".($std_qt+12).",e.S".($std_qt+12).",((e.Lo_Age*100)+e.Hi_Age) as Lo_Hi,e.Sex,e.Stroke,e.Distance,r.SCORE as SCORE,r.Course,m.MName,m.Start,r.Score as conv,1 as pref,If((e.S".($std_qt+12).">0),If(e.S".($std_qt+12).">=r.Score,11,12),13) + If((e.F".($std_qt+12).">0),If(e.F".($std_qt+12)."<=r.Score,0,-10),0) as qt  ";
					    $query.=" From (".$tm4db."result_".$season." as r inner JOIN ".$tm4db."".$std."_".$LSeason." as e on (e.I_R='I'".(($age < $min_age)?'and e.Lo_Hi!=99':'')." and ((r.DISTANCE = e.DISTANCE and r.STROKE=e.STROKE)) and (e.Sex='".$sex."') and e.Lo_Age <= ".$age." AND e.Hi_Age >= ".$age."))";
					    $query.=" inner join ".$tm4db."meet_".$season." as m on (r.Meet = m.Meet ".(($since ==null)?'':"and m.Start >= '".$since."'").((!$restrictbest)?'':" and m.Type='".$type."'").")";
					    $query.=" Where r.NT=0 and r.I_R!='R' and r.Course='L'  and r.ATHLETE =".$athlete;
		
					    //Short course query
					    $query.=" union SELECT r.I_R,e.F".$std_qt.",e.S".$std_qt.",e.F".($std_qt+12).",e.S".($std_qt+12).",((e.Lo_Age*100)+e.Hi_Age) as Lo_Hi,e.Sex,e.Stroke,e.Distance,".course_conversion($tm4db,$season)." as SCORE,r.Course,m.MName,m.Start,".course_conversion($tm4db,$season)." as conv,1 as pref,If((e.S".($std_qt+12).">0),If(e.S".($std_qt+12).">=".course_conversion($tm4db,$season).",11,12),13) + If((e.F".($std_qt+12).">0),If(e.F".($std_qt+12)."<=".course_conversion($tm4db,$season).",0,-10),0) as qt  ";
					    $query.=" From (".$tm4db."result_".$season." as r inner JOIN ".$tm4db."".$std."_".$LSeason." as e on (e.I_R='I'".(($age < $min_age)?'and e.Lo_Hi!=99':'')." and ((r.DISTANCE = e.DISTANCE and r.STROKE=e.STROKE)) and (e.Sex='".$sex."') and e.Lo_Age <= ".$age." AND e.Hi_Age >= ".$age."))";
					    $query.=" inner join ".$tm4db."meet_".$season." as m on (r.Meet = m.Meet ".(($since ==null)?'':"and m.Start >= '".$since."'").((!$restrictbest)?'':" and m.Type='".$type."'").")";
					    $query.=" Where r.NT=0 and r.I_R='I' and r.Course='S'  and r.ATHLETE =".$athlete;
				       ...
				       if($object->Course=='SO')
					 {
					    ...
					    $query.=" SELECT r.I_R,e.F".$std_qt.",e.S".$std_qt.",e.F".($std_qt+12).",e.S".($std_qt+12).",((e.Lo_Age*100)+e.Hi_Age) as Lo_Hi,e.Sex,e.Stroke,e.Distance,r.SCORE,r.Course,m.MName,m.Start,r.Score as conv,1 as pref,If((e.S".$std_qt.">0),If(e.S".$std_qt.">=r.Score,11,12),13) + If((e.F".$std_qt.">0),If(e.F".$std_qt."<=r.Score,0,-10),0) as qt ";
					    $query.=" From (".$tm4db."result_".$season." as r inner JOIN ".$tm4db."".$std."_".$LSeason." as e on (e.I_R='I'".(($age < $min_age)?'and e.Lo_Hi!=99':'')." and ((r.DISTANCE = e.DISTANCE and r.STROKE=e.STROKE)) and (e.Sex='".$sex."') and e.Lo_Age <= ".$age." AND e.Hi_Age >= ".$age."))";
					    $query.=" inner join ".$tm4db."meet_".$season." as m on (r.Meet = m.Meet ".(($since ==null)?'':"and m.Start >= '".$since."'").((!$restrictbest)?'':" and m.Type='".$type."'").")";
					 ...
					 {	//Short course query
					    ...
					    $query.=" SELECT r.I_R,e.F".$std_qt.",e.S".$std_qt.",e.F".($std_qt+12).",e.S".($std_qt+12).",((e.Lo_Age*100)+e.Hi_Age) as Lo_Hi,e.Sex,e.Stroke,e.Distance,r.SCORE,r.Course,m.MName,m.Start,r.SCORE as conv,1 as pref,If((e.S".($std_qt+12).">0) Or (e.S".$std_qt.">0),If(e.S".$std_qt.">0,If(e.S".$std_qt.">=r.Score,11,12),13),13) + If((e.F".$std_qt.">0) Or (e.F".($std_qt+12).">0),If((e.F".$std_qt.">0),If(e.F".$std_qt."<=r.Score,0,-10),10),0) as qt ";
					    $query.=" From (".$tm4db."result_".$season." as r inner JOIN ".$tm4db."".$std."_".$LSeason." as e on (e.I_R='I'".(($age < $min_age)?'and e.Lo_Hi!=99':'')." and ((r.DISTANCE = e.DISTANCE and r.STROKE=e.STROKE)) and (e.Sex='".$sex."') and e.Lo_Age <= ".$age." AND e.Hi_Age >= ".$age."))";
					    $query.=" inner join ".$tm4db."meet_".$season." as m on (r.Meet = m.Meet ".(($since ==null)?'':"and m.Start >= '".$since."'").((!$restrictbest)?'':" and m.Type='".$type."'").")";
					    $query.=" Where r.NT=0 and r.I_R!='R' and r.Course='S'  and r.ATHLETE =".$athlete;
		
					    //long course query
					    $query.=" union SELECT r.I_R,e.F".$std_qt.",e.S".$std_qt.",e.F".($std_qt+12).",e.S".($std_qt+12).",((e.Lo_Age*100)+e.Hi_Age) as Lo_Hi,e.Sex,e.Stroke,e.Distance,r.SCORE,r.Course,m.MName,m.Start,".course_conversion($tm4db,$season)." as conv,2 as pref,If((e.S".($std_qt+12).">0) Or (e.S".$std_qt.">0),If(e.S".($std_qt+12).">0,If(e.S".($std_qt+12).">=r.Score,11,12),13),13) + If((e.F".$std_qt.">0) Or (e.F".($std_qt+12).">0),If((e.F".($std_qt+12).">0),If(e.F".($std_qt+12)."<=r.Score,0,-10),10),0) as qt ";
					    $query.=" From (".$tm4db."result_".$season." as r inner JOIN ".$tm4db."".$std."_".$LSeason." as e on (e.I_R='I'".(($age < $min_age)?'and e.Lo_Hi!=99':'')." and ((r.DISTANCE = e.DISTANCE and r.STROKE=e.STROKE)) and (e.Sex='".$sex."') and e.Lo_Age <= ".$age." AND e.Hi_Age >= ".$age."))";
					    $query.=" inner join ".$tm4db."meet_".$season." as m on (r.Meet = m.Meet ".(($since ==null)?'':"and m.Start >= '".$since."'").((!$restrictbest)?'':" and m.Type='".$type."'").")";
				       ...
				       if($object->Course=='S')  //Covert all times to shrt course then Consider them, note preferance (pref is equal here =1)
					 {
				       ...
				       $query.=" union SELECT null as I_R,e.F".$std_qt.",e.S".$std_qt.",e.F".($std_qt+12).",e.S".($std_qt+12).",((e.Lo_Age*100)+e.Hi_Age) as Lo_Hi,e.Sex,e.Stroke,e.Distance, null as SCORE, null as Course, null as MName, '' as Start, null as conv,13 as pref,IF( ".$qtset." ,33,33) as qt, null as SCORE ";
				       ...
				       while ($object = db_fetch_array($result))
					 {
						 ...
						 $timebox='';
						 if (($object['qt']%10==1 & $object['conv'] !=null) || ($object['qt']%10==2 & $enforce==0))
						 	$timebox.= get_time($object['conv']).' <small>'.$object['Course'].'</small>';
							
						 if (!($object['qt']%10==1 & $object['conv'] !=null))	
						 {
							 if ($timebox!='')
							 $timebox.='<br>'; 
							 if($object['S'.($coursepref=='L'?12:0)]!=null)
						 	$timebox.= '( '.get_time($object['S'.($coursepref=='L'?12:0)]).'<small>'.$coursepref.'</small> )';
						 }
						
						 //$link = 'athlete/entries/events/'.arg(3).'/'.$object->Meet;
						 $rows[] = array('onmouseover'=>"dis_qt(".$i.",".js_null($object['SCORE2']).",".js_null((($object['Course']=='L')?0:1)).",".js_null($object['S'.$std_qt]).",".js_null($object['F'.$std_qt]).",".js_null($object['S'.($std_qt+12)]).",".js_null($object['F'.($std_qt+12)]).",".$multi.")",'data' => array($object['Distance'],Stroke($object['Stroke']),Age($object['Lo_Hi']),array('data'=>$timebox."<div class='cellrel'><div class='cellinfodis' id='s".$i."'></div></div>",'class'=>(($object['qt']%10==1)?'green':(($object['qt']%10==2)?(($enforce==1)?'red':'yellow'):(($object['qt']%10==3)?'red':''))).(($object['qt']<10)?(($enforce==1)?' min_red':' min_yellow'):(($object->qt>30)?' red ':(($object->qt>20)?' min_red':'')))),$object['I_R'],get_date($object['Start']),$object['MName']));
						 $i++;
					 }
