...
//		  db_query("Alter Table ".$tm4db."team_".$season." ADD UNIQUE KEY `ZID` (`Team`), add KEY `LSC` (`LSC`,`TCode`)");
		  ...
		  db_query("Update ".$tm4db."result_".$season."  as p Set p.RBest = 0");
		  ...
		  db_query("update ".$tm4db."result_".$season." as r set r.RBest = if(((@".$grp." != (@".$grp.":=concat(r.athlete,r.COURSE,r.STROKE,r.distance)))),1,0) where (r.NT=0 and r.I_R != 'R') order by r.Athlete,r.Course,r.Stroke,r.Distance,r.score");
...
*/
		  ...
		  $results = db_query("Select distinct Years from fina_points where Years <=".$season." order by Years desc");
		  ...
		  if($object = db_fetch_object($results))
		  {
			  ...
			  $fina_year = $object->Years;
			   ...
			   variable_set('perfanal_'.$season.'_ranking_fina_year', $fina_year);
